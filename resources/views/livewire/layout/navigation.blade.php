...
<nav x-data="{ open: false }" class="bg-zinc-100 dark:bg-zinc-800 border-b border-zinc-100 dark:border-zinc-700">
    ...
    <div class="flex justify-between h-16">
      <div class="flex">
        ...
        <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
          <x-breeze.nav-link :href="route('quotes')" :active="request()->routeIs('quotes')" wire:navigate>
            {{ __('Quotes') }}
          </x-breeze.nav-link>
      ...
      <div class="hidden sm:flex sm:items-center sm:ms-6">
        <x-breeze.dropdown align="right" width="48">
          <x-slot name="trigger">
            <button
              class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-zinc-500 dark:text-zinc-400 bg-zinc-100 dark:bg-zinc-800 hover:text-zinc-700 dark:hover:text-zinc-300 focus:outline-none transition ease-in-out duration-150">
  ...
  <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
    ...
    <div class="pt-4 pb-1 border-t border-zinc-200 dark:border-zinc-600">
      <div class="px-4">
        <div class="font-medium text-base text-zinc-800 dark:text-zinc-200" x-data="{{ json_encode(['name' => auth()->user()->name]) }}" x-text="name"
